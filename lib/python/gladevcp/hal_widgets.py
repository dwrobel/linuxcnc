...
hal_pin_changed_signal = ('hal-pin-changed', (GObject.SignalFlags.RUN_FIRST, None, (GObject.TYPE_OBJECT,)))
...
class HAL_ProgressBar(Gtk.ProgressBar, _HalWidgetBase):
    ...
    def _hal_init(self):
        ...
        if self.yellow_limit or self.red_limit:
            self.set_fraction(0)
            self.modify_bg(Gtk.StateType.PRELIGHT, Gdk.Color('#0f0'))
    ...
    def hal_update(self):
        ...
        if color:
            self.modify_bg(Gtk.StateType.PRELIGHT, Gdk.color_parse(color))
