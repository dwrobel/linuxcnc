...
%if 0%{?rhel}
# There is no gtk2 on the rhel9
%bcond_with classicloader
%else
%bcond_without classicloader
%endif

%{?python_enable_dependency_generator}

%global date 20230401
%global commit0 85305d78b77c06c855a658aef39de2ab91ee3ee2
%global shortcommit0 %(c=%{commit0}; echo ${c:0:7})

Name:          linuxcnc
Version:       2.10.0
Release:       0.94.%{date}git%{shortcommit0}%{?dist}
...
Patch2:   %{name}-0002-Add-halgraph.patch
Patch3:   %{name}-0003-Allow-to-overwrite-default-CXXFLAGS.patch
Patch4:   %{name}-0004-Make-sure-all-compilations-honors-CPPFLAGS.patch
Patch5:   %{name}-0005-Use-TEMP_FAILURE_RETRY-to-handle-EINTR-properly.patch
Patch6:   %{name}-0006-Remove-unused-thread_lock.patch
Patch7:   %{name}-0007-Unify-FIFO_SCHED-between-root-and-non-root-user.patch
Patch8:   %{name}-0008-Always-report-realtime-from-rtapi_is_realtime.patch
Patch9:   %{name}-0009-Disable-using-iptables-work-in-progress.patch
Patch10:   %{name}-0010-rootless-incr_io_usage.patch
Patch11:   %{name}-0011-rootless-incr_mem_usage.patch
Patch12:   %{name}-0012-Remove-direct-usage-of-inb-outb.patch
Patch13:   %{name}-0013-pncconf-Add-PLASMA_DW_34-support-for-7i92-card.patch
Patch14:   %{name}-0014-Add-timings-for-DM860T-stepper-driver.patch
Patch15:   %{name}-0015-Revert-tweak-glBitmap-so-it-works-on-Buster-Bullseye.patch
Patch16:   %{name}-0016-Fix-canterp.so-installation.patch
...
%configure \
    ...
    %{!?with_classicloader:--disable-gtk2} \
