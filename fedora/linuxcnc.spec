...
%global commit0 2fe2838fe1c34172326ff298c062012c7c35a370
%global shortcommit0 %(c=%{commit0}; echo ${c:0:7})

Name:          linuxcnc
Version:       2.9.0
Release:       0.32.%{date}git%{shortcommit0}%{?dist}
...
Patch1:   %{name}-0001-Fix-generating-config-files.patch
Patch2:   %{name}-0002-Add-vectorized-version-of-linuxcnc-icon.patch
Patch3:   %{name}-0003-Improve-searching-LinuxCNC-components-in-GNOME-menu.patch
Patch4:   %{name}-0004-Make-sure-all-compilations-honors-CPPFLAGS.patch
Patch5:   %{name}-0005-rootless-incr_io_usage.patch
Patch6:   %{name}-0006-rootless-incr_mem_usage.patch
...
Patch9:   %{name}-0009-rootless-rtapi_is_realtime-work-in-progress.patch
Patch10:   %{name}-0010-Remove-direct-usage-of-inb-outb.patch
Patch11:   %{name}-0011-Disable-using-iptables.patch
Patch12:   %{name}-0012-Run-tests-in-verbose-mode-and-with-timeout.patch
Patch13:   %{name}-0013-Add-halgraph.patch
Patch14:   %{name}-0014-Fix-python3-compatibility-for-persistence.py-popupke.patch
Patch15:   %{name}-0015-Increase-travis-install-build-deps.sh-interoperabili.patch
...
%{_bindir}/rotarydelta
%{_bindir}/rs274
%caps(cap_sys_rawio,cap_sys_nice=ep) %{_bindir}/rtapi_app
%{_bindir}/scaragui
%{_bindir}/schedrmt
