...
Name:          linuxcnc
Version:       2.10.0
Release:       0.97.%{date}git%{shortcommit0}%{?dist}
...
Patch3:   %{name}-0003-Be-more-tolerant-to-string-in-VERSION-file.patch
Patch4:   %{name}-0004-pncconf-Avoid-using-hardcoded-number-of-pins-24.patch
Patch5:   %{name}-0005-Add-halgraph.patch
Patch6:   %{name}-0006-Allow-to-overwrite-default-CXXFLAGS.patch
Patch7:   %{name}-0007-Make-sure-all-compilations-honors-CPPFLAGS.patch
Patch8:   %{name}-0008-Use-TEMP_FAILURE_RETRY-to-handle-EINTR-properly.patch
Patch9:   %{name}-0009-Remove-unused-thread_lock.patch
Patch10:   %{name}-0010-Unify-FIFO_SCHED-between-root-and-non-root-user.patch
Patch11:   %{name}-0011-Always-report-realtime-from-rtapi_is_realtime.patch
Patch12:   %{name}-0012-Disable-using-iptables-work-in-progress.patch
Patch13:   %{name}-0013-rootless-incr_io_usage.patch
Patch14:   %{name}-0014-rootless-incr_mem_usage.patch
Patch15:   %{name}-0015-Remove-direct-usage-of-inb-outb.patch
Patch16:   %{name}-0016-pncconf-Add-PLASMA_DW_34-support-for-7i92-card.patch
Patch17:   %{name}-0017-Add-timings-for-DM860T-stepper-driver.patch
