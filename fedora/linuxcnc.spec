...
%global date 20231211
%global commit0 cac8b1bb236f7693a637d024bdf2279e67689fe6
%global shortcommit0 %(c=%{commit0}; echo ${c:0:7})

Name:          linuxcnc
Version:       2.10.0
Release:       0.98.%{date}git%{shortcommit0}%{?dist}
...
Patch1:   %{name}-0001-Fix-module-locale-has-no-attribute-format.patch
Patch2:   %{name}-0002-Fix-SyntaxWarning-invalid-escape-sequence.patch
Patch3:   %{name}-0003-SyntaxWarning-invalid-escape-sequence-d.patch
Patch4:   %{name}-0004-Be-more-tolerant-to-string-in-VERSION-file.patch
Patch5:   %{name}-0005-pncconf-Avoid-using-hardcoded-number-of-pins-24.patch
Patch6:   %{name}-0006-Add-halgraph.patch
Patch7:   %{name}-0007-Allow-to-overwrite-default-CXXFLAGS.patch
Patch8:   %{name}-0008-Make-sure-all-compilations-honors-CPPFLAGS.patch
Patch9:   %{name}-0009-Use-TEMP_FAILURE_RETRY-to-handle-EINTR-properly.patch
Patch10:   %{name}-0010-Remove-unused-thread_lock.patch
Patch11:   %{name}-0011-Unify-FIFO_SCHED-between-root-and-non-root-user.patch
Patch12:   %{name}-0012-Always-report-realtime-from-rtapi_is_realtime.patch
Patch13:   %{name}-0013-Disable-using-iptables-work-in-progress.patch
Patch14:   %{name}-0014-rootless-incr_io_usage.patch
Patch15:   %{name}-0015-rootless-incr_mem_usage.patch
Patch16:   %{name}-0016-Remove-direct-usage-of-inb-outb.patch
Patch17:   %{name}-0017-pncconf-Add-PLASMA_DW_34-support-for-7i92-card.patch
Patch18:   %{name}-0018-Add-timings-for-DM860T-stepper-driver.patch
